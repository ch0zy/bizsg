...
// ─── Conversation Viewer (middle pane — v7: all turns stacked, read-only) ──────
function ConversationViewer({ pairs, currentIndex, results, onSelectTurn }) {
  const currentPair = pairs[currentIndex]
  if (!currentPair) return null

  const convTurns = pairs
    .map((p, idx) => ({ pair: p, idx }))
    .filter(({ pair }) => pair._convId === currentPair._convId)
  ...
  return (
    <div style={{
      ...
      {/* Conversation label */}
      <div style={{ padding: `${sp.lg}px ${sp.xl}px ${sp.sm}px`, flexShrink: 0 }}>
        <p style={{ ...type.caption, color: color.typeHelper, textTransform: 'uppercase', letterSpacing: '0.08em', margin: 0 }}>
          {currentPair._convTopic || `Conversation ${currentPair._convIndex + 1}`}
        </p>
      </div>

      {/* Turn cards */}
      <div style={{ padding: `0 ${sp.xl}px ${sp.xl}px`, display: 'flex', flexDirection: 'column', gap: sp.lg }}>
        {convTurns.map(({ pair, idx }) => {
          const isSelected = idx === currentIndex
          const result = results[pair.case_id]

          return (
            <div
              key={pair.case_id}
              onClick={() => !isSelected && onSelectTurn(idx)}
              style={{
                borderRadius: 8,
                border: `1.5px solid ${isSelected ? color.primary : color.borderLighter}`,
                background: color.bgSurface,
                overflow: 'hidden',
                cursor: isSelected ? 'default' : 'pointer',
                transition: 'border-color 0.15s',
                boxShadow: isSelected ? `0 0 0 3px ${brand[50]}22` : 'none',
              }}
            >
              {/* Turn header */}
              <div style={{
                ...
                borderBottom: `1px solid ${color.borderLighter}`,
                background: isSelected ? `${brand[10]}` : color.bgOverlay,
              }}>
                <span style={{
                  ...type.caption, fontWeight: 600,
                  color: isSelected ? color.primary : color.typeHelper,
                  textTransform: 'uppercase', letterSpacing: '0.08em',
                }}>
                  Turn {pair._turnIndex + 1}
                </span>
                {pair.channel && (
                  <span style={{
                    ...type.caption, color: color.typeHelper,
                ...
                {result && (
                  <span style={{
                    ...type.caption, marginLeft: 'auto', fontWeight: 600,
                    color: result.verdict === 'pass' ? '#1E771E' : '#DC2626',
                  }}>
                    {result.verdict === 'pass' ? '✓ Passed' : '✗ Flagged'}
                  </span>
                )}
              </div>

              {/* Input */}
              <div style={{ padding: `${sp.md}px ${sp.lg}px`, borderBottom: `1px solid ${color.borderLighter}` }}>
                <p style={{ ...type.caption, color: color.typeHelper, textTransform: 'uppercase', letterSpacing: '0.08em', margin: `0 0 ${sp.xs}px` }}>
                  Input
                </p>
                <p style={{ ...type.labelMdReg, color: color.typeBodyLight, lineHeight: '22px', margin: 0 }}>
                  {pair.question}
                </p>
              </div>
              ...
              <div style={{ padding: `${sp.md}px ${sp.lg}px` }}>
                ...
                {pair.answer_text?.trim() ? (
                  <p style={{ ...type.labelMdReg, color: color.typeBodyLight, lineHeight: '22px', margin: 0 }}>
                    {pair.answer_text}
                  </p>
                ) : (
                  <p style={{ ...type.labelMdReg, color: color.typeBodyLight, fontStyle: 'italic', lineHeight: '22px', margin: 0 }}>
