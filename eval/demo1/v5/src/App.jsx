...
const type = {
  ...
  caption:       { fontSize: 11, fontWeight: 500, lineHeight: '16px', letterSpacing: '0.06em' },
}
              ...
              if (tile.kind === 'added') {
                return (
                  <div key={`added-${tile.id}`} style={{
                    ...
                    <p style={{ ...type.labelMdReg, lineHeight: '20px', flex: 1, color: tile.text.trim() ? '#166534' : color.typeBodyLight, fontStyle: tile.text.trim() ? 'normal' : 'italic' }}>
...
// ─── Claim Row (v3) ───────────────────────────────────────────────────────────
function ClaimRow({ index, claim, agency, isOwn, state, onUpdate }) {
...
function ClaimReviewPanel({
  ...
  const handleAddClaim = (flatPos) => {
    const next = [...addedClaims]
    next.splice(flatPos, 0, { kind: 'added', id: Date.now(), text: '' })
    onAddedClaimsChange(next)
  }
  ...
  return (
    <div style={{ padding: sp.xl, fontFamily: font, display: 'flex', flexDirection: 'column', gap: 0, height: '100%', overflowY: 'scroll', scrollbarGutter: 'stable' }}>
      ...
      {!sourcesOnly && <div style={{ borderTop: `1px solid ${color.borderLighter}`, marginTop: sp.xl }} />}
      <SourcesSection
        sources={pair.sources}
        sourceStates={sourceStates}
        onSourceStateChange={onSourceStateChange}
        addedSources={addedSources}
        onAddedSourcesChange={onAddedSourcesChange}
        compact={sourcesOnly}
      />

      {/* Submit */}
      <div style={{ paddingTop: sp.lg, marginTop: sp.xl }}>
...
export default function App() {
  ...
  const freshClaimStates = (pair) =>
    pair.claims.map(() => ({ verdict: 'pass', tag: null, whyInaccurate: '', correction: '', citation: '', citationFiles: [], explanation: '' }))
  ...
  const handleClaimsSubmit = useCallback(() => {
    const pair = pairs[currentIndex]
    if (!pair) return
    if (panelState === 'sources-only-claims') {
      const anySourceBad = Object.values(sourceStates).some(s => s.isIncorrect)
      if (anySourceBad) {
        advance({ case_id: pair.case_id, verdict: 'fail', sourceResults: Object.entries(sourceStates).map(([id, s]) => ({ id, ...s })) }, pairs, currentIndex)
      } else {
        setPassFeedback(true)
        setStreak(s => s + 1)
        setShowKeyHint(false)
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', fontFamily: font, background: color.bgOverlay }}>
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, background: color.bgSurface, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          {!passFeedback && (panelState === 'claims' || panelState === 'sources-only-claims') && (
            <ClaimReviewPanel
              pair={currentPair}
              claimStates={claimStates}
              ...
              sourcesOnly={panelState === 'sources-only-claims'}
            />
          )}
        </div>
      </div>
